...
void pathSum(vector<vector<int>> &result, list<int> &cur, BinaryTreeNode *root, int sum);
...
void pathSum(vector<vector<int>> &result, list<int> &cur, BinaryTreeNode *root, int sum) {
	...
	{
		cur.push_back(root->m_nValue);
		pathSum(result, cur, root->m_pLeft, sum - root->m_nValue);
		pathSum(result, cur, root->m_pRight, sum - root->m_nValue);
		cur.pop_back();
	}
}
...
void Test(char* testName, BinaryTreeNode* pRoot, int expectedSum)
{
	...
	// leetcode 113
	vector<vector<int>> ret = pathSum(pRoot, expectedSum);
